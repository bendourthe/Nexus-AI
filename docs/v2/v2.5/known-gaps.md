...
| QG-v251-3 | QG | v2.5.1 Phase 4 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | Implementation-convergence on 2026-09-26 added no new task lines. The smoke host check held (`packaged-smoke.test.ts`, 11 tests). The HTML rendered-surface detector did not run: Python Playwright is not installed (`ModuleNotFoundError`). The adversarial-verifier skill was not invoked. | Install Playwright and run `scripts/detect_visual_defects.py` against the four pillar routes, and run the adversarial-verifier skill. The packaged window probe is not a substitute for those delegates. |

...
| QG-v251-2 | QG | v2.5.1 Phase 3 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | The Windows executable was launched locally and `bundle.smoke.mjs` passed. `.github/workflows/packaged-window-smoke.yml` is on `develop` at `66001961`. GitHub refused `workflow_dispatch` because that file is not on the default branch `main` (HTTP 404). `tauri-driver` still has no macOS build. | Merge the workflow to `main`, then dispatch `packaged-window-smoke.yml`. Do not use `tauri-driver`. |

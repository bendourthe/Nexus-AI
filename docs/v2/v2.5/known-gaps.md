...
| QG-v251-2 | QG | v2.5.1 Phase 3 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | A packaged Tauri window was not launched. The smoke judges a probe report instead. `tauri-driver` has no macOS build, so it was not chosen. `bundle.smoke.mjs` is not a CI job. | On a Windows host, build the desktop bundle, feed its probe report to `desktop/tests/packaged/bundle.smoke.mjs`, and confirm the four selectors. |
| CI-v251-1 | CI | v2.5.1 Phase 4 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | The pipeline comparison was not applied. There is no single aggregate required check and no `fast`/`full`/`platform`/`report`/`release` profiles. Linux and macOS installer rehearsals were dispatched (runs 36153544032 and 36153548528) and have not concluded. | Approve a profile migration separately. Do not rewrite `ci.yml` inside this plan without that approval. Record the rehearsal conclusions when those runs finish. |

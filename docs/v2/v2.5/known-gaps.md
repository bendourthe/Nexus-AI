...
| QG-v251-3 | QG | v2.5.1 Phase 4 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | The Tier 3 deep pass stopped as a bounded result. The smoke host check held against `localhost.evil.com`, `http://127.0.0.1@evil.com/models`, an empty connection, and a non-URL (`packaged-smoke.test.ts`, 11 tests). Rendered-surface delegates and implementation-convergence were not run. | Run those two remaining checks against the plan artifacts and attach their output under `## Tier 3 deep pass`. |
...
| QG-v251-2 | QG | v2.5.1 Phase 3 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | Windows run 36214895318 launched the bundle and the probe printed `{"ok":true,"digest":"4ac3034c06049ced364afc830971fedafb304d60aab0d9b0aac58e084a2fbf79","findings":[]}`. The debug port is compiled into that smoke build only. `tauri-driver` was not used. |

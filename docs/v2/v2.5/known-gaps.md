...
| QG-v251-2 | QG | v2.5.1 Phase 3 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | Local `nexus-shell.exe` passed `bundle.smoke.mjs`. On the Windows runner the shell stays up, but port 9333 never opens. Run 36211349568 died in Node 22 with `EISDIR` on `lstat 'D:'`. Run 36212019248, after starting the sidecar by file name, still had no debug port and no sidecar stderr. | Find why WebView2 on `windows-latest` does not honor `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS`. Do not use `tauri-driver`. |

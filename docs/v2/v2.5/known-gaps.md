...
| QG-v251-3 | QG | v2.5.1 Phase 4 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | The detector's measurement ran on 2026-09-26 against the built shell after in-app navigation. At 1440px, `/chatbot`, `/coding`, `/images`, and `/videos` each showed their route selector. Each route reported `horizontal-overflow` on `aside`. `/images` also reported two `undersized-text-box` findings. Cold start replaces a direct route URL with `/chatbot`, so the detector's file mode cannot open the other routes by itself. | Decide whether the sidebar overflow and the two image labels are defects, then re-measure. |

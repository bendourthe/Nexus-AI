...
| CI-v251-1 | CI | v2.5.1 Phase 4 | `docs/v2/v2.5/plans/v2.5.1-adoption-packaged-verification-and-accessibility.md` | The pipeline comparison was not applied. There is no single aggregate required check and no `fast`/`full`/`platform`/`report`/`release` profiles. Linux and macOS installer workflows are manual dispatch and were not run in this cycle. | Approve a profile migration separately. Do not rewrite `ci.yml` inside this plan without that approval. |
